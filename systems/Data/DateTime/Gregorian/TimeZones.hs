...
-- TODO: Separate codes & name from delta
data TimeZone = TZ
    { code   :: String
    , full   :: String
    , use    :: String
...
instance Eq TimeZone where (==) = (==) `on` (hour &&& minute)
instance Ord TimeZone where (<=) = (<=) `on` (hour &&& minute)
instance Show TimeZone where show = showTimeZone Numeric
...
showTimeZone sty tz = let
    ...
    out = case sty of
        ...
        Minimal -> \c h m ->
            if m == 0 then printf "%c%02d" c h else printf "%c%02d:%02d" c h m
    in out s (hour tz) (minute tz)

instance Coded TimeZone where
    encode tz = toInteger $ hour tz * 100 + minute tz
    decode i = TZ "" "" "" (j `div` 100) (j `mod` 100) where j = fromIntegral i

times :: Int -> (x -> x) -> x -> x
n `times` f = if n > 0 then f . (pred n `times` f) else id

parseTimeZone :: ShowStyle -> Padding -> Parser TimeZone
parseTimeZone Numeric p = decode <$> sizedSignedParser 5 p
parseTimeZone HourMinute p = do
    h <- sizedSignedParser 3 ((3 `times` decrease) p)
    m <- char ':' *> (read <$> count 2 digit)
    pure $ TZ "" "" "" (fromIntegral h) m
parseTimeZone HourMinuteSecond p =
    parseTimeZone HourMinute ((3 `times` decrease) p) <* string ":00"
parseTimeZone Minimal p =
    try (parseTimeZone HourMinute p)
    <|> (TZ "" "" "" . fromIntegral <$> (sizedSignedParser 3 p) <*> pure 0)
...
defaultStr :: [a] -> [[a]] -> [[a]]
defaultStr _ [] = []
defaultStr a (x:xs) = (if null x then a else x) : defaultStr a xs

instance Formattable TimeZone where
    numbers tz = map (`showTimeZone` tz) [minBound..maxBound]
    names tz = defaultStr (showTimeZone HourMinute tz) [code tz, full tz]
    width = const 4
    name = code

instance Loadable TimeZone where
    parseNumber _ i p = encode <$> parseTimeZone (toEnum i) p
    parseName _ 1 c = choice [cased c (full tz) *> pure (encode tz)
        | tz <- timeZones]
    parseName _ _ c = choice [cased c (code tz) *> pure (encode tz)
        | tz <- timeZones]

timeZone :: String -> Maybe TimeZone
timeZone str = find (\tz -> code tz == str) timeZones
